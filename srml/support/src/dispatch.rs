...
macro_rules! decl_module {
	...
	(@impl_deposit_event
		...
		// TODO TODO: ensure $event_trait_instance=$trait_instance and $event_instance=$instance(=Instance)
		// note: a module<T, Instance> may just require an event<T> if its variant only depends on
		// not instanced types
		//
		// TODO TODO: an Event may not depend on the instance but only on the trait it is to the
		// user to specify accordingly.
		//
		// But one the error message will be something like
		// ```
		// error[E0107]: wrong number of type arguments: expected 2, found 1
		//   --> srml/balances/src/lib.rs:174:1
		//    |
		//174 | / decl_module! {
		//175 | |     pub struct Module<T: Trait<Instance>, Instance=DefaultInstance> for enum Call where origin: T::Origin {
		//176 | |         fn deposit_event<T>() = default;
		//177 | |
		//...   |
		//199 | |     }
		//200 | | }
		//    | |_^ expected 2 type arguments
		// ```
		//
		// which is not explicit at all, !! TODO TODO think about that
